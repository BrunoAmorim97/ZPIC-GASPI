...
void check_leaving_particles(t_species* spec, int num_part_to_send[][NUM_ADJ], int fake_part_index[][NUM_ADJ], int part_seg_write_index[NUM_ADJ])
{
	...
	// Add fake particles to each segment
...
#pragma oss declare reduction(	add_current : t_current_reduce : \
								add_thread_current(omp_out, omp_in)) \
								initializer( omp_priv = init_private_current_reduce(omp_orig) )
...
void spec_advance(t_species* spec, t_emf* emf, t_current* current)
{
	...
	#pragma oss task reduction(add_current:current_reduce_out)
	for(int i = 0; i < np; i++)
	{
		...
		dep_current_zamb(spec->part[i].ix, spec->part[i].iy, di, dj, spec->part[i].x, spec->part[i].y, dx, dy, qnx, qny, qvz, current->nrow_local, current_reduce_out.J);
