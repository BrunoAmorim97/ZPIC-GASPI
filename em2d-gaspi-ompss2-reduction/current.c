...
t_current_reduce init_private_current_reduce(t_current_reduce current_reduce_out)
{
	// printf("init_private_current_reduce\n"); fflush(stdout);
	const unsigned int num_cells = current_reduce_out.J_buff_size / sizeof(t_vfld);

	t_current_reduce private_current_reduce = {

		.J_buff = calloc(num_cells, sizeof(t_vfld)),
		.J_buff_num_cells = num_cells,
		.J_buff_size = current_reduce_out.J_buff_size,

		// Make J point to cell [0][0]
		.J = private_current_reduce.J_buff + (current_reduce_out.J - current_reduce_out.J_buff)
	};

	return private_current_reduce;
}
